...
def test_sampling_contrastive_pretraining_dataset_sampling():
    ...
    video_paths = {
        sim: [video_basedir / sim / f"translated_{model}.mp4" for model in models]
    ...
    # Check dataset size
    sim_lengths = [check_num_frames(video_paths[sim][0]) for sim in simulations]
...
def test_sampling_contrastive_pretraining_dataset_data_loading():
    ...
    batch = dataset[0]
    assert batch.shape == (len(models), batch_size, 3, height, width)
