...
def process_spotlight_trial(
    ...
    Note:
        ...
        rotation is stored in JSON files in the same directory. The size
        of these rotated-and-cropped images is specified in the metadata.

        The predicted bodyseg masks have the same size as the input to the
        bodyseg model. The input images to the bodyseg models are taken
        from `cropped_behavior_dir`, but they are resized to a smaller size
        (e.g. 256x256) before being passed to the model. Therefore, upon
        loading the predicted bodyseg masks, the first thing to do is to
        resize them back to the size of images in `cropped_behavior_dir`.
        Only then can we apply the reverse image transform to map the
        bodyseg masks back to the original behavior image space before
    ...
    Args:
        ...
        viz_vmin: Minimum value for visualization.
        viz_vmax: Maximum value for visualization.
    ...
    logging.info(f"Loading bodyseg output from: {bodyseg_output_path}")
    ...
    for muscle_frame_id in muscle_path_by_muscle_frameid.keys():
        ...
        metadata_path_by_muscle_frameid[muscle_frame_id] = input_transform_metadata_path
    ...
    logging.info(
        f"Processing {len(muscle_frame_ids)} muscle frames with {n_workers} workers"
    )
    ...
    for muscle_frame_id in muscle_frame_ids:
        ...
        input_transform_metadata_path = metadata_path_by_muscle_frameid[muscle_frame_id]
        is_first_frame = muscle_frame_id == muscle_frame_ids[0]
        viz_output_path = None
        if visualize_masks:
            viz_output_path = viz_out_dir / f"frame_{muscle_frame_id:06d}.png"
    ...
    # Use joblib for parallel processing (handles n_workers=1 internally)
    logging.info(
        f"Processing {len(muscle_frame_ids)} muscle frames with {n_workers} workers"
    )
    results_all_frames = Parallel(n_jobs=n_workers, verbose=1)(
        delayed(process_single_muscle_frame)(**kwargs)
        for kwargs in tqdm(input_kwargs_all_frames, disable=None, desc="Mapping segmap")
    )

    # Save mapped segmaps and cropped muscle images to H5 files
...
def process_single_muscle_frame(
    segmap: np.ndarray,
    ...
    """Process a single muscle frame (helper function for parallelization).
    See `process_spotlight_trial` for details (including arguments)."""
    # Load muscle image
    ...
    if (
        ...
        and not suppress_size_mismatch_warning
    ):
        logging.warning(
...
if __name__ == "__main__":
    ...
    for spotlight_dir in spotlight_basedir.glob("20250613-fly1b-*"):
        ...
        logging.info(f"Processing spotlight trial: {spotlight_trial}")
