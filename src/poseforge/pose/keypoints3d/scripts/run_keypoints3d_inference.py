...
if __name__ == "__main__":
    ...
    n_workers = prod_config.get("common", {}).get(
        "n_workers", prod_config["keypoints3d"].get("n_workers", 16)
    )
    inference_image_size = tuple(
        prod_config.get("common", {}).get("inference_image_size")
        or prod_config["keypoints3d"]["inference_image_size"]
    )
