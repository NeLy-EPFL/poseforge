...
def visualize_keypoints3d(
    *,
    ...
    ax3d_pan_period_dispsec: bool = 10,  # viewpoint pan period in sec of rendered video
    frame_range: tuple[int, int] | None = None,
) -> None:
    """Visualize 3D keypoint position predictions as video.
    
    The video will have two panels: left panel shows the input recording with predicted
    2D keypoint positions overlaid, and right panel shows the predicted 3D keypoint
    positions as a skeleton. Anatomy-constrained 3D pose will also be shown on the right
    if inverse kinematics output is provided.
    
    Args:
        visualization_output_path: Path for the output video.
        keypoints3d_output_path: Path to the H5 file containing 3D keypoint predictions.
        invkin_output_path: Optional path to the H5 file containing inverse kinematics
            output.
        aligned_behavior_video_path: Path to the aligned behavior video (generated by
            spotlight-tools postprocessing) used fore predictions.
        recording_fps: Frame rate of the original recording (not the aligned video).
        play_speed: Playback speed in the output video (relative to real time).
        plotted_image_size: Size to resample input video to on the left panel.
        loading_batch_size: Batch size for loading video frames.
        loading_n_workers: Number of workers for loading video frames.
        loading_buffer_size: Buffer size for loading video frames.
        loading_cache_video_metadata: Whether to cache metadata for faster loading.
        rendering_n_workers: Number of workers for rendering video frames.
        ax3d_pan_period_dispsec: Period of the panning viewing point for the right 3D
            pose panel, in seconds of the output rendered video.
        frame_range: Optional tuple of (start_frame, end_frame) to specify a subset of
            frames to visualize. If None, visualize all frames.
    """
    ...
    animator = _Keypoints3DAnimator(
        ...
        with_invkin=invkin_output_path is not None,
        ax3d_pan_period_in_frames=recording_fps * play_speed * ax3d_pan_period_dispsec,
    )
...
class _Keypoints3DAnimator(Animator):
    ...
    fwd_kin_line_width = 1
    antenna_marker_size = 5
    x_lim = (0.5, 3.5)
    y_lim = (-3.5, -0.5)
    z_lim = (-0.5, 2.0)
    ax3d_pan_amplitude = 30
    ax3d_elevation = 30  # deg
    xyz_grid_size = 1  # mm
    accessory_color = "#404040"
    ...
    def __init__(
        ...
        *,
        ...
        with_invkin: bool,
        ax3d_pan_period_in_frames: int,
    ) -> None:
        self.image_shape = image_shape
        self.with_invkin = with_invkin
        self.ax3d_pan_period_in_frames = ax3d_pan_period_in_frames

    def setup(self):
        fig, (ax2d, _ax_3d_placeholder) = plt.subplots(1, 2, figsize=(8, 4))
        self.artists = {}
        ...
        ax2d.set_title("Spotlight recording")
        ax2d.axis("off")
        self.artists["input_img"] = ax2d.imshow(
        ...
        self.ax_3d = ax3d  # for updating view later
        ax3d.set_title("3D pose")
        ax3d.set_xlim(*self.x_lim)
        ax3d.set_ylim(*self.y_lim)
        ax3d.set_zlim(*self.z_lim)
        ...
        ax3d.set_xticks(np.arange(*self.x_lim, self.xyz_grid_size), labels=[])
        ax3d.set_yticks(np.arange(*self.y_lim, self.xyz_grid_size), labels=[])
        ax3d.set_zticks(np.arange(*self.z_lim, self.xyz_grid_size), labels=[])
        ax3d.tick_params("both", color=self.accessory_color)
        ax3d.xaxis.line.set_color(self.accessory_color)
        ax3d.yaxis.line.set_color(self.accessory_color)
        ax3d.zaxis.line.set_color(self.accessory_color)
        ax3d.set_xlabel("fore/aft", labelpad=-10, color=self.accessory_color)
        ax3d.set_ylabel("med/lat", labelpad=-10, color=self.accessory_color)
        ax3d.set_zlabel("height", labelpad=-10, color=self.accessory_color)
        ...
        ax3d.text2D(
            0.5,
            -0.05,
            "colored lines: raw predictions\nblack lines: anatomy-constrained",
            transform=ax3d.transAxes,
            ha="center",
            va="top",
            color=self.accessory_color,
        )
    ...
    def update(self, frame_id: int, data: Any):
        ...
        self.ax_3d.view_init(elev=self.ax3d_elevation, azim=azimuth)
