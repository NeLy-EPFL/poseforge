...
`port_slp_labels.py`, or any other `.slp` file with predictions already in a
single consistent coordinate domain) and, for every frame that does not
already have a user-labeled `Instance`, decides whether to add one (linked to
the predicted instance via `from_predicted`) based on:
...
Usage:
    python filter_slp_labels.py --input-path predictions.slp --output-path filtered.slp
"""
...
def main(
    ...
    Args:
        ...
        output_path: Where to save the filtered `.slp` file.
